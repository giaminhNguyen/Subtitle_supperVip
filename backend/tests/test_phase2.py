...
class FakeYouTube:
    ...
    def list_upload_pages(self, playlist_id):
        if playlist_id != self.playlist_id:
            raise PlaylistNotFoundError("gone")
        for start in range(0, len(self.video_ids), self.page_size):
            if self.fail_at_page is not None and self.pages_fetched == self.fail_at_page:
                raise ConnectionError("network down")
            self.pages_fetched += 1
            if self.on_page:
                self.on_page(self.pages_fetched)
...
def make_channel(engine, playlist="PL", cursor=None, complete=False, known=()):
    ...
    channel = Channel(
        ...
        title="K",
        url="https://www.youtube.com/@k",
        uploads_playlist_id=playlist,
        sync_cursor_video_id=cursor,
        history_complete_at=datetime.utcnow() if complete else None,
    )
    channel.settings = ChannelSettings(export_formats=["srt"])
    s.add(channel)
    s.commit()
    if known:
        s.execute(
            insert(Video),
            [
                {"id": f"k-{v}", "channel_id": channel.id, "youtube_video_id": v, "title": v, "url": "u", "video_type": "video", "status": VideoStatus.completed, "retry_count": 0}
                for v in known
            ],
        )
        s.commit()
...
def scan(engine, channel_id, mode="new", **payload):
    s = new_session(engine)()
    job = jobs_service.enqueue(s, "scan", channel_id=channel_id, payload={"mode": mode, **payload})
    s.commit()
    job_id = job.id
    s.close()
...
def videos_of(s):
    return {v.youtube_video_id for v in s.scalars(select(Video))}


def only_run(s):
    s.expire_all()
    return s.scalars(select(SyncRun)).one()
...
def test_new_channel_stores_uploads_playlist_id(shared_db, monkeypatch):
    ...
    with database.SessionLocal() as db:
        assert db.scalar(select(Channel.uploads_playlist_id)) == "UUnew"


def test_cached_playlist_is_not_resolved_again(shared_db, monkeypatch):
    fake = install(monkeypatch, FakeYouTube(["a", "b"]))
    s, channel_id = make_channel(shared_db)
    scan(shared_db, channel_id)
    assert fake.resolve_calls == [] and videos_of(s) == {"a", "b"}


def test_legacy_channel_without_playlist_resolves_once_and_persists(shared_db, monkeypatch):
    fake = install(monkeypatch, FakeYouTube(["a"], playlist_id="UUfound"))
    s, channel_id = make_channel(shared_db, playlist=None)
    scan(shared_db, channel_id)
    drain()
    scan(shared_db, channel_id)
    assert len(fake.resolve_calls) == 1 and fake.resolve_calls[0].endswith("/channel/UC1")
    s.expire_all()
    assert s.get(Channel, channel_id).uploads_playlist_id == "UUfound"
...
def test_stale_playlist_is_re_resolved_exactly_once(shared_db, monkeypatch):
    ...
    s.expire_all()
    assert s.get(Channel, channel_id).uploads_playlist_id == "NEW" and s.get(Job, job_id).status == JobStatus.completed
...
def test_still_invalid_after_re_resolve_does_not_loop(shared_db, monkeypatch):
    ...
    assert job.status == JobStatus.queued and "PlaylistNotFoundError" in job.error


# ---------- incremental scan ----------
def test_first_sync_is_a_full_scan_and_sets_state(shared_db, monkeypatch):
    fake = install(monkeypatch, FakeYouTube([f"v{i}" for i in range(120)]))
    s, channel_id = make_channel(shared_db)
    scan(shared_db, channel_id)
...
def test_legacy_channel_with_videos_but_no_cursor_never_reports_zero_new(shared_db, monkeypatch):
    ...
    fake = install(monkeypatch, FakeYouTube(ids))
...
def test_one_new_video_advances_cursor(shared_db, monkeypatch):
    old = [f"v{i}" for i in range(300)]
    fake = install(monkeypatch, FakeYouTube(["NEW"] + old))
...
def test_many_new_videos_across_page_boundary(shared_db, monkeypatch):
    old = [f"v{i}" for i in range(300)]
    fresh = [f"n{i}" for i in range(60)]
    fake = install(monkeypatch, FakeYouTube(fresh + old))
    s, channel_id = make_channel(shared_db, cursor="v0", complete=True, known=old)
    scan(shared_db, channel_id)
...
def test_duplicate_entries_create_one_video_and_one_job(shared_db, monkeypatch):
    install(monkeypatch, FakeYouTube(["a", "b", "a", "c", "b"], page_size=2))
    s, channel_id = make_channel(shared_db)
    scan(shared_db, channel_id)
...
def test_missing_cursor_falls_back_to_deep_scan_and_recovers(shared_db, monkeypatch):
    ...
    fake = install(monkeypatch, FakeYouTube(["NEW"] + ids))  # old cursor "gone" was deleted
...
def test_since_scan_never_touches_sync_state(shared_db, monkeypatch):
    install(monkeypatch, FakeYouTube(["a", "b"]))
    s, channel_id = make_channel(shared_db, cursor="old", complete=False)
...
def test_failed_scan_does_not_advance_cursor(shared_db, monkeypatch):
    ids = [f"v{i}" for i in range(200)]
    install(monkeypatch, FakeYouTube(NEW60 + ids, fail_at_page=1))  # 60 new: page 2 is needed
    s, channel_id = make_channel(shared_db, cursor="v0", complete=True, known=ids)
    job_id = scan(shared_db, channel_id)
    assert channel_state(s, channel_id) == ("v0", True)
    s.expire_all()
    assert s.get(Job, job_id).status == JobStatus.queued  # will retry; page-1 work is kept, idempotent


def test_scan_retry_after_failure_reuses_run_and_finishes_correctly(shared_db, monkeypatch):
    ids = [f"v{i}" for i in range(200)]
    broken = FakeYouTube(NEW60 + ids, fail_at_page=1)
    install(monkeypatch, broken)
    s, channel_id = make_channel(shared_db, cursor="v0", complete=True, known=ids)
    scan(shared_db, channel_id)
    s.execute(text("UPDATE jobs SET scheduled_at = :t"), {"t": datetime.utcnow() - timedelta(seconds=1)})
    s.commit()
    install(monkeypatch, FakeYouTube(NEW60 + ids))
...
def test_large_history_needs_few_requests(shared_db, monkeypatch):
    known = [f"v{i}" for i in range(5000)]
    new = ["n1", "n2", "n3"]
    fake = install(monkeypatch, FakeYouTube(new + known))
    s, channel_id = make_channel(shared_db, cursor="v0", complete=True, known=known)
    scan(shared_db, channel_id)
    incremental_pages = fake.pages_fetched
    drain()
    fake.pages_fetched = 0
    scan(shared_db, channel_id, mode="all")
    assert incremental_pages == 1 and fake.pages_fetched == 101 and only_run_count(s) == 2


def only_run_count(s):
    s.expire_all()
    return len(s.scalars(select(SyncRun)).all())
...
def test_run_stays_open_until_every_child_job_is_terminal(shared_db, data_dir, monkeypatch):
    install(monkeypatch, FakeYouTube(["v1", "v2", "v3", "v4"]))
    s, channel_id = make_channel(shared_db)
...
def test_all_success_completes_and_all_failed_fails(shared_db, data_dir, monkeypatch):
    install(monkeypatch, FakeYouTube(["v1", "v2"]))
    s, channel_id = make_channel(shared_db)
    outcome_fetch(monkeypatch, {})
    scan(shared_db, channel_id)
    drain()
    assert only_run(s).status == "completed"
    s.execute(text("DELETE FROM job_logs"))
    s.execute(text("DELETE FROM subtitles"))
    s.execute(text("DELETE FROM jobs"))
    s.execute(text("DELETE FROM sync_runs"))
    s.execute(text("DELETE FROM videos"))
    s.commit()
    outcome_fetch(monkeypatch, {"v1": subtitles.BlockedByYouTube("b"), "v2": subtitles.BlockedByYouTube("b")})
    s.execute(text("UPDATE channels SET sync_cursor_video_id=NULL, history_complete_at=NULL"))
    s.commit()
    scan(shared_db, channel_id)
    drain()
...
def test_run_is_scanning_and_unfinished_while_the_scan_is_active(shared_db, monkeypatch):
    ...
    def peek(page):
        with database.SessionLocal() as other:
            run = other.scalars(select(SyncRun)).one()
            seen.append((run.status, run.finished_at))

    install(monkeypatch, FakeYouTube([f"v{i}" for i in range(120)], on_page=peek))
    s, channel_id = make_channel(shared_db)
    scan(shared_db, channel_id)
...
def test_failed_scan_run_reflects_failure_after_retries_are_exhausted(shared_db, monkeypatch):
    ...
    s, channel_id = make_channel(shared_db)
    scan(shared_db, channel_id)
    s.execute(text("UPDATE jobs SET attempts = max_attempts - 1"))
    s.commit()
    s.execute(text("UPDATE jobs SET scheduled_at = :t"), {"t": datetime.utcnow() - timedelta(seconds=1)})
    s.commit()
    assert worker.process_one()
    run = only_run(s)
    assert run.status == "failed" and run.finished_at is not None and "ConnectionError" in run.error


def test_temporary_failure_then_success_counts_success_only(shared_db, data_dir, monkeypatch):
    install(monkeypatch, FakeYouTube(["v1"]))
    s, channel_id = make_channel(shared_db)
    scan(shared_db, channel_id)
    outcome_fetch(monkeypatch, {"v1": ConnectionError("blip")})
    assert worker.process_one()
    run = only_run(s)
    assert (run.failed, run.successful, run.status, run.finished_at) == (0, 0, "downloading", None)
    outcome_fetch(monkeypatch, {})
    s.execute(text("UPDATE jobs SET scheduled_at = :t"), {"t": datetime.utcnow() - timedelta(seconds=1)})
    s.commit()
    assert worker.process_one()
    run = only_run(s)
    assert (run.failed, run.successful, run.status) == (0, 1, "completed")
...
def test_refresh_is_idempotent_and_sets_finished_at_once(shared_db, data_dir, monkeypatch):
    ...
    drain()
    first = only_run(s)
    snapshot = (first.successful, first.status, first.finished_at, first.queued_videos)
    for _ in range(3):
        with database.SessionLocal() as db:
            refresh_sync_run_state(db, first.id, now=datetime.utcnow() + timedelta(days=1))
            db.commit()
    again = only_run(s)
    assert (again.successful, again.status, again.finished_at, again.queued_videos) == snapshot
...
def test_restart_recovers_child_job_without_double_counting(shared_db, data_dir, monkeypatch):
    ...
    with database.SessionLocal() as db:
        claimed = queue.claim_next_job(db, "dead-worker", 60)
    s.execute(text("UPDATE jobs SET lease_expires_at = :t WHERE id = :id"), {"t": datetime.utcnow() - timedelta(seconds=1), "id": claimed.id})
    s.commit()
    drain()
    run = only_run(s)
...
def test_exhausted_stale_child_fails_and_closes_the_run(shared_db, monkeypatch):
    ...
    with database.SessionLocal() as db:
        claimed = queue.claim_next_job(db, "dead-worker", 60)
    s.execute(text("UPDATE jobs SET lease_expires_at = :t, attempts = max_attempts WHERE id = :id"), {"t": datetime.utcnow() - timedelta(seconds=1), "id": claimed.id})
    s.commit()
    with database.SessionLocal() as db:
        queue.recover_stale_jobs(db)
    run = only_run(s)
    assert (run.status, run.failed) == ("failed", 1) and run.finished_at is not None


def test_retrying_a_failed_child_reopens_then_closes_the_run(shared_db, data_dir, monkeypatch):
    install(monkeypatch, FakeYouTube(["v1"]))
    s, channel_id = make_channel(shared_db)
    outcome_fetch(monkeypatch, {"v1": subtitles.BlockedByYouTube("b")})
    scan(shared_db, channel_id)
    drain()
    assert only_run(s).status == "failed" and only_run(s).finished_at is not None
    failed_job = s.scalars(select(Job).where(Job.kind == "download")).one()
    client = TestClient(main.app)
    assert client.post(f"/api/jobs/{failed_job.id}/retry").status_code == 200
    run = only_run(s)
    assert (run.status, run.finished_at, run.failed) == ("downloading", None, 0)
    outcome_fetch(monkeypatch, {})
    drain()
    run = only_run(s)
...
def test_cancelled_child_makes_run_partial_not_stuck(shared_db, data_dir, monkeypatch):
    ...
    drain()
    run = only_run(s)
    assert (run.status, run.successful) == ("partial", 1) and run.finished_at is not None


# ---------- job idempotency (DB level) ----------
def test_one_active_job_per_target_is_a_db_guarantee(shared_db):
    s, channel_id = make_channel(shared_db)
    video = Video(channel_id=channel_id, youtube_video_id="x", title="x", url="u")
    s.add(video)
    s.commit()
    first = jobs_service.enqueue(s, "download", channel_id=channel_id, video_id=video.id)
    s.commit()
...
def test_enqueue_adopts_unowned_active_job_into_the_run(shared_db):
    ...
    video = Video(channel_id=channel_id, youtube_video_id="x", title="x", url="u")
    s.add(video)
    run = SyncRun(channel_id=channel_id, mode="new")
    s.add(run)
    s.commit()
    manual = jobs_service.enqueue(s, "download", channel_id=channel_id, video_id=video.id)
    s.commit()
...
def test_fresh_database_upgrades_to_head_with_indexes(tmp_path, monkeypatch):
    cfg, url = alembic_config(tmp_path, monkeypatch, "fresh.db")
...
def seed_0002(url, duplicate_jobs=False):
    ...
    with engine.begin() as c:
        ...
        c.execute(
            text(
                "INSERT INTO videos (id,channel_id,youtube_video_id,title,url,video_type,status,retry_count,subtitle_path) VALUES ('v1','c1','yt1','t1','u','video','completed',0,'a/b.srt'), ('v2','c1','yt2','t2','u','video','no_subtitle',0,NULL), ('v3','c1','yt3','t3','u','video','blocked',0,NULL)"
            )
        )
        ...
        c.execute(
            text(
                "INSERT INTO jobs (id,kind,status,channel_id,video_id,payload,attempts,max_attempts,scheduled_at,created_at,error) VALUES ('j1','download','completed','c1','v1','{\"sync_run_id\": \"r1\"}',1,5,'2026-01-01','2026-01-01',NULL), ('j2','download','completed','c1','v2','{\"sync_run_id\": \"r1\"}',1,5,'2026-01-01','2026-01-01','no sub'), ('j3','download','failed','c1','v3','{\"sync_run_id\": \"r1\"}',5,5,'2026-01-01','2026-01-01','blocked')"
            )
        )
        if duplicate_jobs:
            c.execute(
                text(
                    "INSERT INTO jobs (id,kind,status,channel_id,video_id,payload,attempts,max_attempts,scheduled_at,created_at) VALUES ('d1','download','queued','c1','v3','{}',0,5,'2026-01-01','2026-01-01'), ('d2','download','queued','c1','v3','{}',0,5,'2026-01-01','2026-01-01')"
                )
            )
...
def test_0002_database_upgrades_keeping_data_and_backfilling_links(tmp_path, monkeypatch):
    cfg, url = alembic_config(tmp_path, monkeypatch, "old.db")
    command.upgrade(cfg, "0002")
    engine = seed_0002(url)
    command.upgrade(cfg, "head")
    with engine.connect() as c:
...
def test_migration_keeps_legacy_duplicate_active_jobs_and_skips_only_that_index(tmp_path, monkeypatch):
    cfg, url = alembic_config(tmp_path, monkeypatch, "dups.db")
    command.upgrade(cfg, "0002")
    engine = seed_0002(url, duplicate_jobs=True)
    command.upgrade(cfg, "head")
    with engine.connect() as c:
...
def test_migrated_legacy_channel_still_syncs(tmp_path, monkeypatch):
    cfg, url = alembic_config(tmp_path, monkeypatch, "legacy.db")
    command.upgrade(cfg, "0002")
    seed_0002(url).dispose()
    command.upgrade(cfg, "head")
