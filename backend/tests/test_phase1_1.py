...
def make_download_job(engine):
    ...
    channel = Channel(youtube_channel_id="UC1", title="K", url="u")
    ...
    s.add(video)
    s.commit()
    run = SyncRun(channel_id=channel.id, mode="new")
    s.add(run)
    s.commit()
    job = Job(kind="download", channel_id=channel.id, video_id=video.id, sync_run_id=run.id)
    s.add(job)
    s.commit()
...
def worker_b_takes_over(engine):
    ...
    s = new_session(engine)()
    s.execute(text("UPDATE jobs SET lease_expires_at=:t"), {"t": datetime.utcnow() - timedelta(seconds=5)})
    s.commit()
...
def assert_untouched_by_a(s, job_id, video_id, run_id):
    ...
    assert (run.successful, run.failed, run.no_subtitle) == (0, 0, 0)
...
def test_finalize_race_lost_lease_between_file_write_and_db_commit(shared_db, data_dir, monkeypatch):
    ...
    fake_transcript(monkeypatch)
    real_write = worker.atomic_write_text
    stolen = []
...
def test_scan_stops_writing_once_ownership_is_lost(shared_db, monkeypatch):
    ...
    s.add(channel)
    s.commit()
    job = Job(kind="scan", channel_id=channel.id, payload={"mode": "new"})
    s.add(job)
    s.commit()
    job_id = job.id

    class FakeClient:
        def resolve_channel(self, _):
            return type("R", (), {"uploads_playlist_id": "UU"})()

        def list_upload_pages(self, _):
            for n in range(3):
                if n == 1:
                    worker_b_takes_over(shared_db)  # lease lost while "fetching" page 2
                yield [
                    {
                        "youtube_video_id": f"v{n}",
                        "metadata": {
                            "youtube_video_id": f"v{n}",
                            "title": "t",
                            "url": "u",
...
def test_timeouts_then_success_keeps_lease_and_finalizes_once(shared_db, monkeypatch):
    ...
    monkeypatch.setattr(ratelimit, "_youtube_limiter", limiter)
    ...
    s, job_id, *_ = make_download_job(shared_db)
    observer = new_session(shared_db)()
    seen = []
...
def test_every_outbound_call_has_explicit_timeouts(monkeypatch, no_wait):
    TIMEOUTS.clear()
    script_httpx(monkeypatch, [FakeResponse(200, {})])
    get()
    timeout = TIMEOUTS[0]
    ...
    subtitles._TimeoutSession().get("https://example.invalid", timeout=3)
...
def test_heartbeat_stops_renewing_after_max_runtime(shared_db):
    ...
    s.expire_all()
    assert s.get(Job, job_id).lease_expires_at < datetime.utcnow() + timedelta(seconds=10)


def test_timing_settings_are_validated():
    with pytest.raises(ValueError):
        Settings(job_lease_seconds=5)
    with pytest.raises(ValueError):
        Settings(job_lease_seconds=60, job_max_runtime_seconds=60)
    with pytest.raises(ValueError):
...
def fk_violations(engine):
    with engine.connect() as c:
        return c.execute(text("PRAGMA foreign_key_check")).fetchall()


def test_foreign_key_check_clean_after_normal_flow(shared_db, data_dir, monkeypatch):
    make_download_job(shared_db)
    fake_transcript(monkeypatch)
...
def test_legacy_orphans_survive_migration_and_are_reported_not_deleted(tmp_path, monkeypatch):
    ...
    with engine.connect() as c:
        assert c.execute(text("SELECT count(*) FROM videos")).scalar() == 1
    engine.dispose()
