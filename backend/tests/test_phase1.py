...
def test_rate_limiter_enforces_requests_per_minute():
    ...
    for _ in range(21): limiter.acquire()
    assert clock.now == pytest.approx(60.0)  # first request immediate, then one every 3s


def test_rate_limiter_zero_disables():
    clock = FakeClock(); limiter = RateLimiter(0, clock=clock.time, sleep=clock.sleep)
    for _ in range(50): limiter.acquire()
    assert clock.sleeps == []


def test_retries_also_count_against_rate_limit():
    clock = FakeClock(); limiter = RateLimiter(20, clock=clock.time, sleep=clock.sleep); attempts = []
    def flaky():
        attempts.append(clock.now)
        if len(attempts) < 3: raise ConnectionError("x")
...
def test_retry_is_bounded_with_growing_backoff(monkeypatch, no_wait):
    ...
    with pytest.raises(youtube.YouTubeError): get()
...
def test_quota_403_not_retried_but_rate_limit_403_is(monkeypatch, no_wait):
    quota = {"error": {"errors": [{"reason": "quotaExceeded"}]}}; limited = {"error": {"errors": [{"reason": "rateLimitExceeded"}]}}
...
def test_api_key_never_leaks_in_errors(monkeypatch, no_wait):
    ...
    with pytest.raises(youtube.YouTubeError) as caught: get()
...
def test_worker_does_not_sleep_while_queue_has_jobs():
    results = iter([True, True, True, False, False]); sleeps = []; calls = []
    def process(): calls.append(1); return next(results)
...
def add_job(session, **kwargs):
    job = Job(kind="download", **kwargs); session.add(job); session.commit(); return job.id


def test_only_one_worker_claims_a_job(shared_db):
    Session = new_session(shared_db)
    s = Session(); add_job(s); s.close()
    barrier = threading.Barrier(6); winners = []
    def attempt(name):
        session = Session(); barrier.wait()
        try:
            if queue.claim_next_job(session, name, 60): winners.append(name)
...
def test_claim_skips_future_and_paused_jobs(shared_db):
    ...
    add_job(s, scheduled_at=datetime.utcnow() + timedelta(hours=1)); add_job(s, status=JobStatus.paused)
...
def test_stale_job_out_of_attempts_fails_instead_of_looping(shared_db):
    ...
    queue.recover_stale_jobs(s); s.expire_all()
...
def test_worker_abandons_result_when_lease_was_taken(shared_db, monkeypatch):
    s = new_session(shared_db)(); job_id = add_job(s)
    def steal(*_):
        thief = new_session(shared_db)(); thief.execute(text("UPDATE jobs SET worker_id='other'")); thief.commit(); thief.close()
...
def test_legacy_key_imported_into_db(shared_db, monkeypatch):
    ...
    with database.SessionLocal() as db: assert db.get(AppSetting, "youtube_api_key").value == "LEGACYKEY12345"
...
def test_legacy_reader_skips_placeholder(tmp_path, monkeypatch):
    env = tmp_path / ".env"; env.write_text("YOUTUBE_API_KEY=replace_me\n", encoding="utf-8")
    monkeypatch.setattr(config, "ENV_FILE", env); monkeypatch.setattr(settings, "youtube_api_key", "")
...
def test_relative_path_roundtrip_and_relocation(tmp_path, monkeypatch, data_dir):
    file = data_dir / "chan" / "vid" / "vi.srt"; file.parent.mkdir(parents=True); file.write_text("x")
    stored = storage.to_stored_path(file)
    assert stored == "chan/vid/vi.srt"
    assert storage.resolve_subtitle_path(stored) == file.resolve()
    moved = tmp_path / "B" / "data"; (moved / "chan" / "vid").mkdir(parents=True)
...
def test_legacy_absolute_paths(tmp_path, data_dir):
    inside = data_dir / "c" / "v.srt"; inside.parent.mkdir(); inside.write_text("x")
    assert storage.resolve_subtitle_path(str(inside)) == inside.resolve()
    outside = tmp_path / "elsewhere.srt"; outside.write_text("x")
    assert storage.resolve_subtitle_path(str(outside)) == outside.resolve()  # legacy file still readable
    with pytest.raises(storage.UnsafePathError): storage.resolve_subtitle_path(str(tmp_path / "gone.srt"))
    with pytest.raises(storage.UnsafePathError): storage.to_stored_path(outside)


@pytest.mark.parametrize("bad", ["../../secret.txt", "chan/../../secret.txt"])
def test_path_traversal_rejected(tmp_path, data_dir, bad):
    (tmp_path / "secret.txt").write_text("secret")
    with pytest.raises(storage.UnsafePathError): storage.resolve_subtitle_path(bad)


# ---------- atomic write ----------
def test_atomic_write_success_leaves_no_temp(data_dir):
    target = data_dir / "c" / "v.srt"; storage.atomic_write_text(target, "xin chào")
...
def test_atomic_write_failure_keeps_old_file(data_dir, monkeypatch):
    target = data_dir / "v.srt"; target.write_text("old", encoding="utf-8")
    def boom(*_): raise OSError("disk full")
    monkeypatch.setattr(storage.os, "replace", boom)
    with pytest.raises(OSError): storage.atomic_write_text(target, "new")
...
def test_upgrade_old_database_keeps_data_and_relativises_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(settings, "database_url", url); monkeypatch.setattr(settings, "data_dir", data)
    ...
    with engine.begin() as c:
        ...
        c.execute(text("INSERT INTO videos (id,channel_id,youtube_video_id,title,url,video_type,status,retry_count,subtitle_path) VALUES ('v1','c1','yt','t','u','video','completed',0,:p)"), {"p": str(inside)})
        ...
        c.execute(text("INSERT INTO jobs (id,kind,status,payload,attempts,max_attempts,scheduled_at,created_at) VALUES ('j1','download','processing','{}',1,5,'2026-01-01','2026-01-01')"))
