...
def test_rescan_does_not_duplicate_video(db, monkeypatch):
    ...
    class FakeClient:
        def resolve_channel(self, _):
            return type("R", (), {"uploads_playlist_id": "UU1"})()
    ...
    jobs_service.scan_channel(db, channel, "new")
    db.commit()
    jobs_service.scan_channel(db, channel, "new")
    db.commit()
...
def test_enqueue_prevents_duplicate_active_job(db):
    channel = Channel(youtube_channel_id="UC2", title="K", url="https://youtube.com/channel/UC2")
    channel.settings = ChannelSettings()
