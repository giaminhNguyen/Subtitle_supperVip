...
def seed_channel(engine, videos=0, jobs=0, logs=0):
    ...
    for i in range(videos):
        s.add(Video(channel_id=channel.id, youtube_video_id=f"v{i:03}", title=f"video {i}", url="u", published_at=BASE + timedelta(days=i)))
    for i in range(jobs):
        s.add(Job(id=f"job{i:03}", kind="download", status=JobStatus.failed, created_at=BASE + timedelta(minutes=i)))
    for i in range(logs):
        s.add(JobLog(id=f"log{i:03}", message=f"m{i}", created_at=BASE + timedelta(seconds=i)))
    s.commit()
    return s, channel.id


def test_jobs_pages_are_stable_complete_and_report_total(shared_db):
    seed_channel(shared_db, jobs=5)
    client = TestClient(main.app)
    first = client.get("/api/jobs?limit=2&offset=0")
    second = client.get("/api/jobs?limit=2&offset=2")
    third = client.get("/api/jobs?limit=2&offset=4")
...
def test_limit_is_bounded(shared_db):
    client = TestClient(main.app)
    for url in ("/api/jobs?limit=501", "/api/jobs?limit=0", "/api/jobs?offset=-1", "/api/logs?limit=1000"):
...
def test_channel_videos_paginate_and_language_filter_does_not_duplicate_rows(shared_db):
    ...
    for fmt in ("srt", "txt", "vtt"):
        s.add(Subtitle(video_id=video.id, language="Vietnamese", language_code="vi", format=fmt, file_path=f"a.{fmt}"))
    s.commit()
    client = TestClient(main.app)
...
def test_logs_paginate_and_filter_by_job(shared_db):
    seed_channel(shared_db, jobs=1, logs=4)
    client = TestClient(main.app)
    response = client.get("/api/logs?limit=3")
    assert response.headers["X-Total-Count"] == "4" and [entry["id"] for entry in response.json()] == ["log003", "log002", "log001"]
...
def test_retry_refuses_when_another_active_job_exists_for_the_target(shared_db):
    ...
    new = Job(id="new", kind="download", status=JobStatus.queued, channel_id=channel_id, video_id=video.id)
    s.add_all([old, new])
    s.commit()
    client = TestClient(main.app)
    assert client.post("/api/jobs/old/retry").status_code == 409
    s.expire_all()
    assert s.get(Job, "old").status == JobStatus.failed
    new.status = JobStatus.completed
    s.commit()
    assert client.post("/api/jobs/old/retry").status_code == 200
    s.expire_all()
    retried = s.get(Job, "old")
    assert (retried.status, retried.attempts, retried.error) == (JobStatus.queued, 0, None)
