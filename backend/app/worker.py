...
def process_download(db, job: Job) -> DownloadResult:
    ...
    if not video or not channel:
        raise RuntimeError("Video hoặc channel không tồn tại")
    options = channel.settings
    guard_job(db, job.id, WORKER_ID, job.attempts)
    video.status, video.last_processed_at = VideoStatus.processing, datetime.utcnow()
    ...
    for fmt in options.export_formats:
        path = folder / f"{transcript.language_code}.{fmt}"
        atomic_write_text(path, serialize(snippets, fmt))
        stored[fmt] = to_stored_path(path)
    atomic_write_text(
        ...
        json.dumps(
            {
                ...
                "subtitle": {"language": transcript.language, "language_code": transcript.language_code, "generated": transcript.is_generated, "translated": translated},
                "files": list(stored.values()),
            },
            ensure_ascii=False,
            indent=2,
        ),
        ...
        if not previous:
            db.add(
                Subtitle(
                    ...
                    is_translated=result.translated,
                    format=fmt,
                    file_path=stored,
                )
            )
        else:
            previous.file_path = stored
    ...
    try:
        ...
        with Heartbeat(SessionLocal, job.id, WORKER_ID, settings.job_lease_seconds, settings.job_max_runtime_seconds):
            try:
                ...
                db.rollback()
                logger.warning("Job %s đã bị worker khác tiếp quản; bỏ toàn bộ kết quả", job.id)
        return True
    finally:
        STATE.idle()
        db.close()
...
def _run_job(db, job: Job):
    ...
    def guard(**values):
        guard_job(db, job_id, WORKER_ID, attempts, **values)

    def log_job(message: str, level: str = "info"):
        db.add(JobLog(job_id=job_id, message=message, level=level))

    def video_row():
        return db.get(Video, video_id) if video_id else None
    ...
    try:
        ...
        if job.kind == "scan":
            if not run_id:  # first attempt (or a job queued before SyncRun linkage existed); a retry reuses the same run
                run = SyncRun(channel_id=job.channel_id, mode=job.payload.get("mode", "new"), status=SyncRunStatus.scanning.value)
                db.add(run)
                db.flush()  # row must exist before the job FK points at it
                guard(sync_run_id=run.id)
                db.commit()
                run_id = run.id
            channel = db.get(Channel, job.channel_id)
            ...
            result = process_download(db, job)
        else:
            raise RuntimeError(f"Loại job không hỗ trợ: {job.kind}")

        def done():
            if isinstance(result, DownloadResult):
                apply_download_result(db, video_id, result)
            if isinstance(result, ScanResult):
                apply_scan_result(db, channel, result)
                log_job(result.summary(channel.title, job.payload.get("mode", "new")))
            log_job("Hoàn tất")
        ...
        def apply():
            v = video_row()
            v.status, v.last_error, v.last_processed_at = VideoStatus.no_subtitle, reason, now()
            log_job("Video không có subtitle")
        ...
        def apply():
            v = video_row()
            v.status, v.last_error, v.last_processed_at = VideoStatus.language_unavailable, reason, now()
            log_job("Không có ngôn ngữ yêu cầu")
        ...
        def apply():
            v = video_row()
            v.status, v.last_error = VideoStatus.blocked, reason
            log_job("YouTube đã chặn request", "error")
        ...
        if attempts < max_attempts:
            ...
            def apply():
                v = video_row()
                if v:
                    v.retry_count, v.last_error, v.last_processed_at, v.status = v.retry_count + 1, error, now(), VideoStatus.queued
            ...
            def apply():
                v = video_row()
                if v:
                    v.retry_count, v.last_error, v.last_processed_at, v.status = v.retry_count + 1, error, now(), VideoStatus.failed
...
def worker_loop(process=process_one, sleep=time.sleep, poll_seconds: float | None = None, should_stop=lambda: False, maintenance=None, schedule=None):
    ...
    while not should_stop():
        ...
        try:
            ...
            logger.exception("Worker gặp lỗi không mong đợi")
            worked = False
        if not worked:
            sleep(poll)
