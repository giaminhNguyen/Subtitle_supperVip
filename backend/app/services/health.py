...
def diagnostics(db: Session) -> dict:
    ...
    info = {
        "app": {"version": APP_VERSION, "python": platform.python_version(), "uptime_seconds": int(time.time() - STARTED_AT)},
    ...
    try:
        ...
        storage.update(free_bytes=usage.free, total_bytes=usage.total)
    except OSError:
        storage.update(free_bytes=None, total_bytes=None)
    ...
    info["active_sync_runs"] = [
        {
            ...
            "queued": r.queued_videos,
            "successful": r.successful,
            "failed": r.failed,
        }
        for r in db.scalars(
            select(SyncRun).where(SyncRun.status.in_([SyncRunStatus.scanning.value, SyncRunStatus.downloading.value])).order_by(SyncRun.started_at.desc()).limit(20)
        )
    ]
