...
def _prune_job_logs(db: Session, cutoff: datetime) -> int:
    ...
    while True:
        ...
        db.execute(delete(JobLog).where(JobLog.id.in_(ids))); db.commit(); total += len(ids)
...
def _prune_runs(db: Session, cutoff: datetime) -> int:
    ...
    total = 0
    while True:
        run_ids = db.scalars(select(SyncRun.id).where(SyncRun.status.in_(TERMINAL_RUNS), SyncRun.finished_at.is_not(None), SyncRun.finished_at < cutoff)
                             .where(~exists().where(Job.sync_run_id == SyncRun.id, Job.status.notin_(TERMINAL_JOBS))).limit(BATCH)).all()
        if not run_ids: return total
        job_ids = db.scalars(select(Job.id).where(Job.sync_run_id.in_(run_ids))).all()
        for start in range(0, len(job_ids), BATCH): _delete_jobs(db, job_ids[start:start + BATCH])
        ...
        db.commit(); total += gone.rowcount
        if gone.rowcount == 0: return total  # nothing deletable in this batch; avoid spinning
...
def _prune_standalone_jobs(db: Session, cutoff: datetime) -> int:
    ...
    total = 0
    while True:
        ids = db.scalars(select(Job.id).where(Job.sync_run_id.is_(None), Job.status.in_(TERMINAL_JOBS), Job.created_at < cutoff, (Job.finished_at.is_(None)) | (Job.finished_at < cutoff)).limit(BATCH)).all()
        if not ids: return total
        _delete_jobs(db, ids); db.commit(); total += len(ids)
...
def run_retention(db: Session, now: datetime | None = None) -> dict:
    ...
    result = {"runs": _prune_runs(db, history), "jobs": _prune_standalone_jobs(db, history),
    ...
    if any(result.values()): logger.info("Retention: %s", result)
    return result
...
class MaintenanceSchedule:
    ...
    def done(self): self._last = self._clock()
