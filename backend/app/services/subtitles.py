...
class BlockedByYouTube(Exception):
    pass
...
def choose_transcript(transcripts, languages: list[str], preference: str, allow_translation: bool):
    ...
    ordered = (
        [x for x in items if not getattr(x, "is_generated", False)]
        if preference == "manual"
        else [x for x in items if getattr(x, "is_generated", False)]
        if preference == "auto"
        else items
    )
    if not ordered:
        ordered = items if preference == "any" else []
    if not ordered:
        raise LanguageUnavailable("Không có loại subtitle được chọn")
    for language in languages:
        if language == "original":
            return ordered[0], False
        for transcript in ordered:
            if getattr(transcript, "language_code", "").lower().split("-")[0] == language.lower().split("-")[0]:
            ...
            if language != "original":
                for transcript in ordered:
                    if getattr(transcript, "is_translatable", False):
                        return transcript.translate(language), True
...
def fetch_selected(video_id: str, languages: list[str], preference: str, allow_translation: bool):
    try:
        api = transcript_api()
        transcript, translated = choose_transcript(
            call_with_retry(lambda: api.list(video_id), category="transcript.list", is_retryable=_transcript_retryable), languages, preference, allow_translation
        )
        ...
        return (
            ...
            [
                {
                    "text": x.text if hasattr(x, "text") else x["text"],
                    "start": x.start if hasattr(x, "start") else x["start"],
                    "duration": x.duration if hasattr(x, "duration") else x.get("duration", 0),
                }
                for x in snippets
            ],
        )
        ...
        if "ipblocked" in name or "requestblocked" in name or "too many" in message or "429" in message:
            raise BlockedByYouTube(str(exc)) from exc
        if "notranscript" in name or "transcriptsdisabled" in name:
            raise SubtitleUnavailable(str(exc)) from exc
        raise
...
def available_transcripts(video_id: str) -> list[dict]:
    ...
    try:
        ...
        return [{"language": t.language, "language_code": t.language_code, "is_generated": t.is_generated, "is_translatable": t.is_translatable} for t in tracks]
    except Exception as exc:
        name = exc.__class__.__name__.lower()
        message = str(exc).lower()
        if "ipblocked" in name or "requestblocked" in name or "too many" in message or "429" in message:
...
def serialize(snippets: list[dict], fmt: str) -> str:
    ...
    if fmt == "csv":
        result = StringIO()
        writer = csv.DictWriter(result, fieldnames=["start", "duration", "text"])
        writer.writeheader()
