...
def _is_retryable(exc: Exception) -> tuple[bool, float | None]:
    if isinstance(exc, YouTubeHTTPError):
        return exc.retryable, exc.retry_after
...
def parse_channel_url(url: str) -> tuple[str, str]:
    ...
    if m := CHANNEL_RE.search(clean):
        return "id", m.group(1)
    if m := HANDLE_RE.search(clean):
        return "handle", "@" + m.group(1)
    if m := USER_RE.search(clean):
        return "username", m.group(1)
    if re.search(r"youtube\.com/(c|[\w.-]+)", clean, re.I):
        return "search", clean.rsplit("/", 1)[-1]
    raise ValueError("URL kênh YouTube không hợp lệ")


class YouTubeDataClient:
    def __init__(self, key: str | None = None):
        self.key = key if key is not None else get_youtube_api_key()

    def _get(self, resource: str, params: dict) -> dict:
        if not self.key:
            raise YouTubeError("Chưa cấu hình YOUTUBE_API_KEY")

        def request() -> dict:
            response = httpx.get(f"{API}/{resource}", params={**params, "key": self.key}, timeout=request_timeout())
            if response.status_code >= 400:
                raise YouTubeHTTPError(
                    response.status_code, self._redact(f"YouTube Data API {response.status_code}: {response.text[:300]}"), _error_reason(response), _retry_after(response)
                )
    ...
    def resolve_channel(self, url: str) -> ResolvedChannel:
        ...
        if key == "id":
            ...
            if not found:
                raise YouTubeError("Không tìm thấy kênh")
            params["id"] = found[0]["snippet"]["channelId"]
        items = self._get("channels", params).get("items", [])
        if not items:
            raise YouTubeError("Không tìm thấy kênh hoặc kênh không công khai")
        item = items[0]
    ...
    def list_upload_pages(self, uploads_playlist_id: str):
        ...
        while True:
            try:
                ...
                if exc.status == 404 and exc.reason == "playlistNotFound":
                    raise PlaylistNotFoundError(str(exc)) from None
                raise
    ...
    def _video_details(self, ids: list[str]) -> dict:
        ...
        for x in items:
            ...
            out[x["id"]] = {
                "youtube_video_id": x["id"],
                "title": s["title"],
                "url": f"https://www.youtube.com/watch?v={x['id']}",
                "published_at": isoparse(s["publishedAt"]).replace(tzinfo=None),
