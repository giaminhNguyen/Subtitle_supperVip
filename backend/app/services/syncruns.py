...
def refresh_sync_run_state(db: Session, run_id: str | None, now: datetime | None = None) -> str | None:
    ...
    for status, outcome, n in db.execute(select(Job.status, Job.outcome, func.count()).where(Job.sync_run_id == run_id, Job.kind == "download").group_by(Job.status, Job.outcome)):
        total += n
        if status in ACTIVE_JOBS: active += n
        elif status == JobStatus.cancelled: cancelled += n
    ...
    if scan_active: status = SyncRunStatus.scanning
    elif active: status = SyncRunStatus.downloading
    elif not scan_ok: status = SyncRunStatus.partial if good else SyncRunStatus.failed
    elif bad == 0: status = SyncRunStatus.completed
    else: status = SyncRunStatus.partial if good else SyncRunStatus.failed

    for column, value in counts.items(): setattr(run, column, value)
