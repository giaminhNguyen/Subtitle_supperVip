...
def enqueue(db: Session, kind: str, *, channel_id: str | None = None, video_id: str | None = None, payload: dict | None = None, sync_run_id: str | None = None) -> Job:
    ...
    if video_id:
        ...
        if video and video.status != VideoStatus.completed:
...
def _scan_pages(db: Session, channel: Channel, playlist_id: str, client, mode: str, since: datetime | None, checkpoint, run_id: str | None) -> ScanResult:
    ...
    for page in client.list_upload_pages(playlist_id):
        ...
        entries = []
        for entry in page:
            if entry["youtube_video_id"] not in seen:
                seen.add(entry["youtube_video_id"])
                entries.append(entry)
        if result.first_video_id is None and page:
            result.first_video_id = page[0]["youtube_video_id"]
        ...
        if fresh:
            db.execute(sqlite_insert(Video).on_conflict_do_nothing(index_elements=["channel_id", "youtube_video_id"]), fresh)
        ...
        for entry in entries:
            ...
            if trusted:  # boundary bookkeeping runs on ids alone, so private/deleted entries still count
                if video_id == cursor:
                    cursor_found, known_run = True, 0
                elif cursor_found and entry["metadata"] is not None:
                    known_run = known_run + 1 if video_id in known else 0
            if not wanted(entry):
                continue
            ...
            if eligible and video.status != VideoStatus.completed:
                enqueue(db, "download", channel_id=channel.id, video_id=video.id, sync_run_id=run_id)
                result.queued += 1
        ...
        checkpoint()
        db.commit()  # commit before the next page's network call
...
def apply_scan_result(db: Session, channel: Channel, result: ScanResult):
    ...
    if result.advance_cursor:
        ...
        if result.history_verified:
            channel.history_complete_at = now
...
def due_channel_syncs(db: Session):
    ...
    for channel in due:
        enqueue(db, "scan", channel_id=channel.id, payload={"mode": "new", "scheduled": True})
