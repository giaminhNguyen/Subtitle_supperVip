...
def _copy_database(source: Path, destination: Path) -> None:
    ...
    try:
        ...
        if not _integrity_ok(temp):
            raise BackupError("Bản sao không qua kiểm tra integrity_check")
...
def _unique_name(directory: Path, stem: str, now: datetime) -> Path:
    candidate = directory / f"{stem}{now:%Y%m%d-%H%M%S}.sqlite3"
    n = 0
    while candidate.exists():
        n += 1
        candidate = directory / f"{stem}{now:%Y%m%d-%H%M%S}-{n}.sqlite3"
    return candidate
...
def backup_database(db_path: Path | None = None, directory: Path | None = None, keep: int | None = None, now: datetime | None = None) -> Path | None:
    ...
    directory = directory or settings.db_backup_dir
    if db_path is None or not db_path.is_file() or db_path.stat().st_size == 0:
        return None
    try:
        if not _has_tables(db_path):
            return None
...
def pre_migration_backup(db_path: Path | None = None, **kwargs) -> Path | None:
    ...
    db_path = db_path or sqlite_path()
    if db_path is None or not db_path.is_file() or not migrations_pending(db_path):
        return None
    ...
    try:  # a live worker refreshes its heartbeat every few seconds
        ...
        try:
            ...
            if row and row[0]:
                ...
                if datetime.utcnow() - last < timedelta(seconds=settings.worker_stale_seconds):
                    raise RestoreError("Worker vẫn đang chạy; hãy chạy stop-app.bat trước.")
        except sqlite3.OperationalError:
            pass  # older DB without the heartbeat table
        finally:
            conn.close()
...
def restore_database(backup: Path, db_path: Path | None = None, directory: Path | None = None, now: datetime | None = None) -> dict:
    ...
    directory = directory or settings.db_backup_dir
    if db_path is None:
        raise RestoreError("Chỉ hỗ trợ SQLite")
    if not backup.is_file():
        raise RestoreError("Không tìm thấy file backup")
    try:
        if not _integrity_ok(backup) or not _has_tables(backup):
            raise RestoreError("Backup không hợp lệ hoặc bị hỏng")
    ...
    if db_path.is_file() and db_path.stat().st_size > 0:
        try:
            if _has_tables(db_path):
                directory.mkdir(parents=True, exist_ok=True)
                safety = _unique_name(directory, SAFETY_PREFIX, now or datetime.now())
                _copy_database(db_path, safety)
        except (sqlite3.Error, OSError) as exc:
    ...
    if not _integrity_ok(db_path):
        raise RestoreError("DB sau restore không qua integrity_check")
...
def rotate_logs(log_dir: Path | None = None, retention_days: int | None = None, now: datetime | None = None) -> dict:
    ...
    log_dir = log_dir or RUNTIME_LOG_DIR
    retention_days = retention_days or settings.runtime_log_retention_days
    now = now or datetime.now()
    archived = deleted = 0
    if not log_dir.is_dir():
        return {"archived": 0, "deleted": 0}
    for path in list(log_dir.glob("*.log")):
        if ARCHIVED_LOG_RE.match(path.name) or path.stat().st_size == 0:
            continue
        stamp = datetime.fromtimestamp(path.stat().st_mtime)
        try:
            path.rename(path.with_name(f"{path.stem}.{stamp:%Y%m%d-%H%M%S}.log"))
            archived += 1
        except OSError:
            pass  # still open by a running process: never touch the current log
