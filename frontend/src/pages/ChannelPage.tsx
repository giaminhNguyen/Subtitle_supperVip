...
export function ChannelPage({ channel, back }: { channel: Channel; back: () => void }) {
  ...
  const queue = (key: string, request: () => Promise<unknown>) =>
    action.run(key, request, 'Đã thêm vào hàng đợi.').then((ok) => {
      if (ok) videos.refresh();
    });
  const saveSettings = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    void action.run(
      'settings',
      () =>
        api.updateSettings(channel.id, {
          preferred_languages: String(form.get('languages'))
            .split(',')
  ...
  if (!detail.data)
    return (
      <main>
        ...
        <ErrorNotice message={detail.error ?? 'Không tải được kênh'} />
      </main>
    );
  ...
  return (
    <main>
      ...
      <div className="actions">
        <button disabled={action.busy} onClick={() => void queue('scan-all', () => api.scan(channel.id, 'all'))}>
          Quét toàn bộ
        </button>
        <button disabled={action.busy} onClick={() => void queue('sync', () => api.syncNew(channel.id))}>
          Đồng bộ video mới
        </button>
        <button className="secondary" disabled={action.busy} onClick={() => void queue('scan-retry', () => api.scan(channel.id, 'retryable'))}>
          Xử lý lỗi / thiếu sub
        </button>
      </div>
      ...
      <section className="settings">
        ...
        <form onSubmit={saveSettings}>
          <label>
            Ưu tiên ngôn ngữ
            <input name="languages" defaultValue={settings.preferred_languages.join(', ')} />
          </label>
          <label>
            Loại sub
            <select name="kind" defaultValue={settings.subtitle_preference}>
          ...
          <label>
            Đồng bộ mỗi (giờ)
            <input name="interval" type="number" min="1" max="720" defaultValue={settings.sync_interval_hours} />
          </label>
          <label className="check">
            <input name="translate" type="checkbox" defaultChecked={settings.allow_translation} /> Cho phép dịch YouTube
          </label>
          <fieldset>
            <legend>Xuất</legend>
            {EXPORT_FORMATS.map((format) => (
              <label className="check" key={format}>
                <input type="checkbox" name="format" value={format} defaultChecked={settings.export_formats.includes(format)} />
                {format.toUpperCase()}
              </label>
            ))}
          </fieldset>
      ...
      <section>
        <div className="section-title">
          ...
          <select
            aria-label="Lọc trạng thái video"
            value={status}
            onChange={(event) => {
            ...
            <tbody>
              {page?.items.map((video) => (
                <tr key={video.id}>
                  ...
                  <td>
                    {video.retry_count}
                    {video.last_error && <small className="error">{video.last_error}</small>}
                  </td>
                  <td>
                    <button
                      className="secondary"
                      disabled={action.busy || IN_PROGRESS.includes(video.status)}
                      onClick={() => void queue(`dl-${video.id}`, () => api.downloadVideo(video.id, true))}
                    >
                      Tải lại
                    </button>
                  </td>
