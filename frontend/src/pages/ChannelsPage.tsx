...
const METRICS: [string, keyof DashboardStats][] = [
  ['Kênh', 'channels'], ['Tổng video', 'videos'], ['Đã có sub', 'completed'], ['Lỗi / Blocked', 'failed'], ['Không có sub', 'no_subtitle'], ['Job đang chờ', 'running_jobs'],
];

export function ChannelsPage({ onSelect, onOpenDiagnostics }: { onSelect: (channel: Channel) => void; onOpenDiagnostics: () => void }) {
  const [url, setUrl] = useState('');
  const action = useAction();
  const overview = usePolling(async (signal) => {
  ...
  const add = async (event: React.FormEvent) => {
    ...
    if (await action.run('add', () => api.addChannel(url))) { setUrl(''); overview.refresh(); }
  };
  const { stats, channels, youtube } = overview.data ?? {};

  return (
    <main>
      <header><div><p className="eyebrow">QUẢN LÝ CAPTION</p><h1>YouTube Subtitle Manager</h1></div><StatusPill onOpen={onOpenDiagnostics} /></header>
      <ApiKeySetup configured={youtube?.configured ?? false} onSaved={overview.refresh} />
      <section className="metrics">{METRICS.map(([label, key]) => <div className="metric" key={key}><small>{label}</small><strong>{stats?.[key] ?? '-'}</strong></div>)}</section>
      <section className="add">
        <div><h2>Thêm kênh YouTube</h2><p>Dán URL dạng @handle, /channel/ID, /user/ hoặc custom URL.</p></div>
        <form onSubmit={add}>
      ...
      <section>
        <div className="section-title"><h2>Kênh đang quản lý</h2><button className="secondary" onClick={overview.refresh} disabled={overview.refreshing}>{overview.refreshing ? 'Đang tải…' : 'Làm mới'}</button></div>
        ...
        <div className="channels">
          ...
          {channels?.map((channel) => (
            <button className="channel" onClick={() => onSelect(channel)} key={channel.id}>
              ...
              <span><b>{channel.title}</b><small>{channel.video_count} video · thêm {new Date(channel.added_at).toLocaleDateString('vi-VN')}</small></span>
              <em>Chi tiết →</em>
            </button>
          ))}
