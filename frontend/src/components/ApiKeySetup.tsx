...
export function ApiKeySetup({ configured, onSaved }: { configured: boolean; onSaved: () => void }) {
  ...
  return (
    <section className="settings api-key-settings">
      ...
      <form onSubmit={save}>
        <label>API key<input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} minLength={10} autoComplete="off" placeholder="AIza..." required /></label>
