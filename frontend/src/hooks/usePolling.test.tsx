...
describe('usePolling', () => {
  it('loads immediately, then polls after each completed request', async () => {
    ...
    await flush();
    expect(result.current.data).toBe('a'); expect(result.current.loading).toBe(false);
    await advance(1000); expect(result.current.data).toBe('b');
    await advance(1000); expect(fetcher).toHaveBeenCalledTimes(3);
  });
  ...
  it('pauses while the tab is hidden and refreshes right away when it comes back', async () => {
    ...
    act(() => setHidden(true));
    await advance(10_000); expect(fetcher).toHaveBeenCalledTimes(1);
    act(() => setHidden(false)); await flush(); expect(fetcher).toHaveBeenCalledTimes(2);
    await advance(1000); expect(fetcher).toHaveBeenCalledTimes(3);
  });

  it('stops everything on unmount: no timers, aborted request, no state update', async () => {
    let signal: AbortSignal | undefined; let resolve: (value: string) => void = () => undefined;
    const fetcher = vi.fn((s: AbortSignal) => { signal = s; return new Promise<string>((done) => { resolve = done; }); });
    ...
    const listeners = vi.spyOn(document, 'removeEventListener'); unmount(); listeners.mockRestore();
  });
  ...
  it('unmount while idle clears the pending timer and the visibility listener', async () => {
    ...
    await flush(); expect(vi.getTimerCount()).toBe(1);
    unmount();
    expect(vi.getTimerCount()).toBe(0);
  ...
  it('supports adaptive intervals and null (no auto refresh)', async () => {
    ...
    await flush(); await advance(1000); expect(fetcher).toHaveBeenCalledTimes(2);
    await advance(60_000); expect(fetcher).toHaveBeenCalledTimes(2); // went idle: polling stopped
  });

  it('a stale response from previous deps never overwrites newer data', async () => {
    const resolvers: Record<string, (value: string) => void> = {};
    const fetcher = vi.fn((_signal: AbortSignal, page: number) => new Promise<string>((done) => { resolvers[`p${page}`] = done; }));
    const { result, rerender } = renderHook(({ page }) => usePolling((signal) => fetcher(signal, page), null, [page]), { initialProps: { page: 1 } });
    rerender({ page: 2 });
    await act(async () => { resolvers.p2('page 2'); });
    await act(async () => { resolvers.p1('page 1 (stale)'); });
  ...
  it('manual refresh while a request is running runs exactly one follow-up', async () => {
    ...
    const fetcher = vi.fn(() => new Promise<string>((done) => { resolvers.push(done); }));
    ...
    expect(result.current.data).toBe('second'); expect(fetcher).toHaveBeenCalledTimes(2);
  });
  ...
  it('reports errors without stopping the polling loop', async () => {
    ...
    const { result } = renderHook(() => usePolling(fetcher, 1000, []));
    await flush(); expect(result.current.error).toBe('boom');
    await advance(1000); expect(result.current.data).toBe('ok'); expect(result.current.error).toBeNull();
  });
});
